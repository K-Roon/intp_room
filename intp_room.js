...
function response(room, msg, sender, isGroupChat, replier, imageDB, packageName) {
  ...
  if (msg.startsWith("보이스룸 종료")) {
    replier.reply("보이스룸이 종료되었습니다. 모두 수고하셨습니다!");
    return;
  }
  ...
  var avg1 = r1.length ? r1.reduce(function(x, y) {
  return x + y;
}) / r1.length : Infinity;
  var avg2 = r2.length ? r2.reduce(function(x, y) {
  return x + y;
}) / r2.length : Infinity;
  return avg1 - avg2;
});
      for (var i = 0; i < keys.length; i++) {
        ...
        var avg = stats.ranks.length ? (stats.ranks.reduce(function(a, b) {
  return a + b;
}) / stats.ranks.length).toFixed(2) : "N/A";
