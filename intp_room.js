...
var QUIZ_BANK = [
  {
    ...
    q: "소가 시끄러우면? (2글자)",
    a: ["소란"],
    hint: "왜 이렇게 _ _ 이야!! 시끄러워 죽겠네!! (욕 아님)"
  }, {
    q: "달에서 쓰는 언어는?",
    a: ["문어"],
...
function startQuizRound(replier) {
  if (!isQuizTime()) {
    safeReply(replier, "⏰ 지금은 퀴즈 가능 시간이 아닙니다. 퀴즈를 정리하고 준비하고 있으니 기다려주세요!\n[퀴즈 가능 시간] 매일 00:00 ~ 23:30");
    return false;
  }
  if (quizActive) {
...
function response(room, msg, sender, isGroupChat, replier, imageDB, packageName) {
  ...
  if (quizActive) {
    ...
    if (msg.startsWith("!")) {
      ...
      // 그 외 모든 !커맨드 차단
      safeReply(replier, "⛔️퀴즈모드가 켜졌습니다.\n다른 명령어는 모두 차단됩니다.🚧");
      return;
    }
  }
  ...
  if (msg == "!") {
    safeReply(replier,
      "📌 채팅봇 기능안내\n"
      + "1. 아침에 채팅치면 자동으로 출석!🎉\n"
      + "2. !출석 – 출석 가능시간 알아보기(혹은 강제출석)\n"
      + "3. !출석랭킹 – 오늘 출석 목록 📋\n" 
      + "4. !출석통계 – 누적 출석 통계 📊\n"
      + "5. !나 – 내 정보 확인하기🧑‍💻\n"
      + "6. !남 (닉네임) – 남의 정보 확인하기👤\n"
      + "7. !멤버통계 – 멤버 종합 통계(점수 순) 📈\n"
      + "8. !밥 / !디저트: 음식 추천 제공 🍲🍨\n"
      + "9. !명언 / !운세: 명언 또는 운세 보기👀\n"
      + "10. !퀴즈 – 넌센스 퀴즈를 맞춰보세요! / !퀴즈랭킹 – 가장 많이 퀴즈를 맞힌 사람은 누구일까요?\n"
      + "문제 발생 시 관리자에게 문의 부탁드립니다. (Version 3.1)");
    return;
  }
